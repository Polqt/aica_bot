...
export function useResumeBuilder() {
  ...
  const loadResumeData = useCallback(async () => {
    try {
      ...
      const [educationData, experienceData, skillsData, summaryData] = await Promise.all([
  ...
  const addEducation = useCallback(async (educationData: UserEducationCreate) => {
    setSaving(true);
    try {
  ...
  const updateEducation = useCallback(async (id: string, educationData: Partial<UserEducationCreate>) => {
    setSaving(true);
    try {
      const updatedEducation = await apiClient.updateEducation(id, educationData);
  ...
  const addExperience = useCallback(async (experienceData: UserExperienceCreate) => {
    setSaving(true);
    try {
  ...
  const updateExperience = useCallback(async (id: string, experienceData: Partial<UserExperienceCreate>) => {
    setSaving(true);
    try {
      const updatedExperience = await apiClient.updateExperience(id, experienceData);
      setExperience(prev => prev.map(exp => exp.id === id ? updatedExperience : exp));
  ...
  const updateSkill = useCallback(async (id: string, skillData: Partial<UserSkillCreate>) => {
    setSaving(true);
    try {
      const updatedSkill = await apiClient.updateSkill(id, skillData);
      setSkills(prev => prev.map(skill => skill.id === id ? updatedSkill : skill));
  ...
  const resetResume = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to reset resume';
      setError(errorMessage);
      toast.error(errorMessage);
      throw err;
    } finally {
